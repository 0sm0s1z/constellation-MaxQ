(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const o of r)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function t(r){const o={};return r.integrity&&(o.integrity=r.integrity),r.referrerPolicy&&(o.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?o.credentials="include":r.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(r){if(r.ep)return;r.ep=!0;const o=t(r);fetch(r.href,o)}})();const $="curl -fsSL https://raw.githubusercontent.com/0sm0s1z/constellation-MaxQ/main/install.sh | bash",x="https://github.com/0sm0s1z/constellation-MaxQ";function R(){const s=(location.hash||"#home").replace("#","");return["home","stack","router","cue","crew","install","invariants","ops"].includes(s)?s:"home"}const L=()=>`
  ...
  <figure class="bezel ${a}">
    ...
    </tbody></table>`}const O=[[203,166,247],[250,179,135],[137,220,235],[180,190,254]];function F(s){const e=s.getContext("2d");if(!e)return()=>{};let t=0,a=0,r=[],o=[],l=[],m=0,f=0,h=0,d=0,u=0,b=0;const y=window.matchMedia("(prefers-reduced-motion: reduce)").matches,k=()=>{const n=Math.min(window.devicePixelRatio||1,2);t=window.innerWidth,a=window.innerHeight,s.width=Math.floor(t*n),s.height=Math.floor(a*n),s.style.width=`${t}px`,s.style.height=`${a}px`,e.setTransform(n,0,0,n,0,0),A()},A=()=>{const n=Math.floor(t*a/11e3);r=Array.from({length:n},()=>({x:Math.random()*t,y:Math.random()*a,z:.25+Math.random()*.75,tw:Math.random()*Math.PI*2,rgb:O[Math.floor(Math.random()*O.length)]})),o=Array.from({length:16},()=>({x:Math.random()*t,y:Math.random()*a})),l=[];for(let i=0;i<o.length;i++){const w=o.map((c,g)=>({j:g,d:Math.hypot(c.x-o[i].x,c.y-o[i].y)})).filter(c=>c.j!==i).sort((c,g)=>c.d-g.d);for(const c of w.slice(0,2))c.d<Math.min(t,a)*.28&&l.push([i,c.j])}},S=n=>{y||(u=(n.clientX/t-.5)*6,b=(n.clientY/a-.5)*6)},M=()=>{f+=y?0:.002,h+=(u-h)*.04,d+=(b-d)*.04,e.setTransform(Math.min(window.devicePixelRatio||1,2),0,0,Math.min(window.devicePixelRatio||1,2),0,0),e.clearRect(0,0,t,a);const n=e.createRadialGradient(t*.5,a*.18,20,t*.5,a*.45,Math.max(t,a));n.addColorStop(0,"#1e1e2e"),n.addColorStop(.55,"#181825"),n.addColorStop(1,"#11111b"),e.fillStyle=n,e.fillRect(0,0,t,a),e.save(),e.translate(h,d),e.strokeStyle="rgba(203,166,247,0.10)",e.lineWidth=1;for(const[i,w]of l)e.beginPath(),e.moveTo(o[i].x,o[i].y),e.lineTo(o[w].x,o[w].y),e.stroke();for(const i of o)e.fillStyle="rgba(180,190,254,0.55)",e.beginPath(),e.arc(i.x,i.y,1.4,0,Math.PI*2),e.fill();for(const i of r){const w=y?.7:.4+.6*Math.abs(Math.sin(f*.9+i.tw)),[c,g,q]=i.rgb;e.fillStyle=`rgba(${c},${g},${q},${.16+i.z*.5*w})`,e.fillRect(i.x,i.y,i.z*1.5,i.z*1.5)}e.restore(),m=requestAnimationFrame(M)};return k(),M(),window.addEventListener("resize",k),window.addEventListener("mousemove",S),()=>{cancelAnimationFrame(m),window.removeEventListener("resize",k),window.removeEventListener("mousemove",S)}}const I={home:{label:"maxq",draw:H},stack:{label:"stack",draw:N},router:{label:"router",draw:Q},cue:{label:"cue",draw:B},crew:{label:"crew",draw:z},install:{label:"install",draw:D},invariants:{label:"invariants",draw:j},ops:{label:"ops",draw:U}},V=["home","stack","router","cue","crew"];function W(s,e){return`
    <header class="topbar">
