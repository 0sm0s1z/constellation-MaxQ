(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const o of r)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function t(r){const o={};return r.integrity&&(o.integrity=r.integrity),r.referrerPolicy&&(o.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?o.credentials="include":r.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(r){if(r.ep)return;r.ep=!0;const o=t(r);fetch(r.href,o)}})();const $="curl -fsSL https://raw.githubusercontent.com/0sm0s1z/constellation-MaxQ/main/install.sh | bash",x="https://github.com/0sm0s1z/constellation-MaxQ";function R(){const s=(location.hash||"#home").replace("#","");return["home","stack","router","cue","crew","install","invariants","ops"].includes(s)?s:"home"}const L=()=>`
  ...
  <figure class="bezel ${a}">
    ...
    </tbody></table>`}const O=[[203,166,247],[250,179,135],[137,220,235],[180,190,254]];function F(s){const e=s.getContext("2d");if(!e)return()=>{};let t=0,a=0,r=[],o=[],l=[],m=0,f=0,h=0,d=0,u=0,w=0;const y=window.matchMedia("(prefers-reduced-motion: reduce)").matches,k=()=>{const n=Math.min(window.devicePixelRatio||1,2);t=window.innerWidth,a=window.innerHeight,s.width=Math.floor(t*n),s.height=Math.floor(a*n),s.style.width=`${t}px`,s.style.height=`${a}px`,e.setTransform(n,0,0,n,0,0),A()},A=()=>{const n=Math.floor(t*a/11e3);r=Array.from({length:n},()=>({x:Math.random()*t,y:Math.random()*a,z:.25+Math.random()*.75,tw:Math.random()*Math.PI*2,rgb:O[Math.floor(Math.random()*O.length)]})),o=Array.from({length:16},()=>({x:Math.random()*t,y:Math.random()*a})),l=[];for(let i=0;i<o.length;i++){const b=o.map((c,g)=>({j:g,d:Math.hypot(c.x-o[i].x,c.y-o[i].y)})).filter(c=>c.j!==i).sort((c,g)=>c.d-g.d);for(const c of b.slice(0,2))c.d<Math.min(t,a)*.28&&l.push([i,c.j])}},S=n=>{y||(u=(n.clientX/t-.5)*6,w=(n.clientY/a-.5)*6)},M=()=>{f+=y?0:.002,h+=(u-h)*.04,d+=(w-d)*.04,e.setTransform(Math.min(window.devicePixelRatio||1,2),0,0,Math.min(window.devicePixelRatio||1,2),0,0),e.clearRect(0,0,t,a);const n=e.createRadialGradient(t*.5,a*.18,20,t*.5,a*.45,Math.max(t,a));n.addColorStop(0,"#1e1e2e"),n.addColorStop(.55,"#181825"),n.addColorStop(1,"#11111b"),e.fillStyle=n,e.fillRect(0,0,t,a),e.save(),e.translate(h,d),e.strokeStyle="rgba(203,166,247,0.10)",e.lineWidth=1;for(const[i,b]of l)e.beginPath(),e.moveTo(o[i].x,o[i].y),e.lineTo(o[b].x,o[b].y),e.stroke();for(const i of o)e.fillStyle="rgba(180,190,254,0.55)",e.beginPath(),e.arc(i.x,i.y,1.4,0,Math.PI*2),e.fill();for(const i of r){const b=y?.7:.4+.6*Math.abs(Math.sin(f*.9+i.tw)),[c,g,q]=i.rgb;e.fillStyle=`rgba(${c},${g},${q},${.16+i.z*.5*b})`,e.fillRect(i.x,i.y,i.z*1.5,i.z*1.5)}e.restore(),m=requestAnimationFrame(M)};return k(),M(),window.addEventListener("resize",k),window.addEventListener("mousemove",S),()=>{cancelAnimationFrame(m),window.removeEventListener("resize",k),window.removeEventListener("mousemove",S)}}const I={home:{label:"maxq",draw:H},stack:{label:"stack",draw:N},router:{label:"router",draw:Q},cue:{label:"cue",draw:B},crew:{label:"crew",draw:z},install:{label:"install",draw:D},invariants:{label:"invariants",draw:j},ops:{label:"ops",draw:U}},V=["home","stack","router","cue","crew"];function W(s,e){return`
    <header class="topbar">
    ...
    </footer>`}function X(s){s.querySelectorAll("button.copy").forEach(e=>{e.addEventListener("click",async()=>{const t=e.dataset.copy??"";try{await navigator.clipboard.writeText(t),e.textContent="copied",e.classList.add("ok"),window.setTimeout(()=>{e.textContent="copy",e.classList.remove("ok")},1400)}catch{e.textContent="fail"}})})}function Y(s){const e=[...s.querySelectorAll("[data-tab]")],t=[...s.querySelectorAll("[data-panel]")];if(!e.length)return;const a=r=>{e.forEach(o=>o.classList.toggle("active",o.dataset.tab===r)),t.forEach(o=>{const l=o.dataset.panel===r;o.classList.toggle("active",l),o.hidden=!l})};e.forEach(r=>{r.addEventListener("click",()=>a(r.dataset.tab??"router"))})}function Z(s){const e=s.querySelector("[data-carousel]");if(!e)return;const t=e.hasAttribute("data-carousel-lock"),a=[...e.querySelectorAll(".slide")],r=[...e.querySelectorAll("[data-dot]")];let o=0,l=0;const m=h=>{o=(h%a.length+a.length)%a.length,a.forEach((d,u)=>{const w=u===o;d.classList.toggle("is-on",w),d.hidden=!w}),r.forEach((d,u)=>d.classList.toggle("is-on",u===o))},f=()=>{t||(window.clearInterval(l),l=window.setInterval(()=>m(o+1),4200))};r.forEach(h=>h.addEventListener("click",()=>{m(Number(h.dataset.dot)),t||f()})),t||(e.addEventListener("mouseenter",()=>window.clearInterval(l)),e.addEventListener("mouseleave",f),new IntersectionObserver(d=>{d.some(u=>u.isIntersecting)?f():window.clearInterval(l)},{threshold:.35}).observe(e)),m(0)}function T(){const s=document.getElementById("app");if(!s)return;const e=R();s.innerHTML=W(I[e].draw(),e),X(s),Y(s),Z(s)}function E(){const s=document.getElementById("loader");if(!s)return;if(window.matchMedia("(prefers-reduced-motion: reduce)").matches){s.remove();return}s.classList.add("out"),window.setTimeout(()=>s.remove(),480)}const P=document.getElementById("stars");P instanceof HTMLCanvasElement&&F(P);T();window.addEventListener("hashchange",T);document.readyState==="complete"?E():window.addEventListener("load",E);
