...
- The home page is the product explanation, not just the hero. `#how` steps, `#kit` tiles (`KIT` + `src/icons` pack), `#trust` (`OWNS` / `REFUSES` / `PROOF`) must match `README.md` and `docs/*.md`. Change the docs first, then the page. Do not invent tools or claims. Kit marks are real logos recolored with `currentColor`, not letter monograms.
- The site is about MaxQ. Topbar = `products ▾` (Router, MaxQ, Cue, Crew, Stack — mirrored by `#surfaces` at the foot of home) + the three why pages `#access` `#control` `#telemetry`. `#why` on home is the navigator into them, right above `#surfaces`. Why copy lives in `src/why.ts`, the index in `WHY` (`src/pages.ts`); the facts are in `docs/VISION.md` → "Why MaxQ". Keep the sheet's real tabs as the ceiling for claims: theme · proxy · firewall · vault · oauth · skills · resources · triggers. Packages is inventory, not install.
- Rhythm: each numbered beat is two blocks. `01` = `#desk` beat + `#kit`; `02` = `#door` beat + `#console`. `#console` is the operator sheet as live type (`DESKTOPS` / `PROCS` in `pages.ts`, `bindConsole` in `main.ts`): desktops cycle, meters breathe, kill drops RAM, proxy flips, intercept stays locked. Its numbers start at the EVA capture (14 / 22 live, RAM 69%). Do not replace it with a screenshot; do not let it claim a control the API does not have.
