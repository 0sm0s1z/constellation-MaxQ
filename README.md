...
Control API listens on loopback only (`http://127.0.0.1:7432/`). `maxq apply` starts `$HOME/bin/maxq-api` (pidfile); `maxq revert` stops it. The API serves a thin Catppuccin Mocha settings sheet with status, apply/revert, and GOST proxy on/off only. It never writes Chrome proxy policy or `cxn-egress.json`.
...
The control API (`$HOME/bin/maxq-api`) binds `127.0.0.1:7432` (override with a loopback address in `$HOME/.config/maxq/api.toml`). Source lives in `cmd/maxq-api` (stdlib HTTP + embedded `ui/`). See [docs/API.md](docs/API.md).
