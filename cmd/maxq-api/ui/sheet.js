(() => {
  ...
  function renderVault(list) {
    ...
    $("st-vault").textContent = pending.length ? pending.length + " pending \xB7 bot claims via POST /vault/credentials/{id}/claim" : "Empty \xB7 paste above for an ephemeral bot handoff";
    ...
    pending.forEach((c) => {
      ...
      text.append(strong);
      const detail = document.createElement("small");
      detail.textContent = c.purpose + " \xB7 expires " + c.expires_at + " \xB7 id " + c.id;
      text.append(detail);
      const actions = document.createElement("div");
      actions.className = "row";
      ...
      cancel.addEventListener("click", () => act(async () => {
        await deleteJSON("/vault/credentials/" + encodeURIComponent(c.id));
      }));
  ...
  function renderVaultUnavailable(message) {
    $("st-vault").textContent = "Unavailable \xB7 " + message;
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    $("vault-form").addEventListener("submit", (event) => {
      ...
      const purpose = $("vault-purpose").value;
