...
async function getEntitlements() { return requestJSON("/entitlements"); }
...
function mapNetworkAuthError(raw) {
...
function showNetworkJoinError(reason) {
...
function syncClearAuthKeyButton() {
    const btn = $("btn-network-clear-key");
...
function renderNetwork(network) {
    ...
    if (network.status)
        parts.push(network.status === "down" ? "Disconnected" : "Up");
    if (network.mode === "headscale" && network.login_server)
        parts.push(network.login_server);
    if (network.auth_key_configured)
        parts.push("auth key stored");
    clearNetworkError();
...
function renderNetworkUnavailable(message) {
    ...
    clearNetworkError();
    ["network-mode", "network-login-server", "network-auth-key", "btn-network-save", "btn-network-leave", "btn-network-clear-key"].forEach((id) => { $(id).disabled = true; });
...
function renderHAUnavailable(message) {
    ...
    ["ha-entity-id", "ha-entity-label", "btn-ha-add", "btn-ha-save"].forEach((id) => { $(id).disabled = true; });
}
...
function addEntitlementFromInputs() {
    ...
    if (existing)
        ...
        operatorEntitlements.push(entry);
    value.value = "";
    label.value = "";
    showMsg("");
    renderEntitlementRows();
...
function renderConnections(connections) {
    ...
    for (const c of connections) {
        ...
        text.append(name);
        const detail = document.createElement("small");
        detail.textContent = c.base_url + (c.auth_configured ? " · auth configured" : "");
        text.append(detail);
        const remove = document.createElement("button");
        remove.type = "button";
        remove.className = "remove";
...
function renderDesktops(desktops, errors) {
    ...
    for (const d of desktops) {
        ...
        source.textContent = String(d.box_identity || d.connection_name || "Unknown box") + " · " + String(d.connection_name || d.source_api || "API");
...
function busy(on) {
    busyState = on;
    ["btn-apply", "btn-revert", "btn-proxy-on", "btn-proxy-off", "btn-network-save", "btn-network-clear-key", "btn-network-leave", "btn-ha-add", "btn-ha-save", "btn-entitlement-add", "btn-entitlement-save"].forEach((id) => { $(id).disabled = on; });
...
async function refresh() {
    ...
    if (entitlements.status === "fulfilled")
        renderEntitlements(entitlements.value);
    else {
    ...
    if (connections.status === "fulfilled")
        renderConnections(connections.value.connections);
    else {
        const message = errorText(connections.reason);
        renderConnectionsUnavailable(message);
        errors.push("connections: " + message);
    }
    if (desktops.status === "fulfilled")
        renderDesktops(desktops.value.desktops, desktops.value.errors || []);
    else {
