# MaxQ theme pack (Catppuccin Mocha)

Copied to `$HOME/.config/maxq/theme/src` on install/apply.

| Path | What |
| --- | --- |
| wallpaper/mocha.png | MaxQ Mocha wallpaper (generated, Catppuccin palette) |
| vendor/catppuccin-mocha-mauve-gtk.zip | Official catppuccin/gtk v1.0.3 Mocha Mauve |
| vendor/catppuccin-mocha-mauve-cursors.zip | Official catppuccin/cursors v2.0.0 Mocha Mauve |
| chrome-mocha/ | Official catppuccin/chrome v5.0.0 Mocha (google-store) |
| ghostty/catppuccin-mocha.conf | Official catppuccin/ghostty Mocha |

Apply installs into `$HOME/.local/share` and `$HOME/.config`. Revert removes MaxQ-owned copies only.
