#!/usr/bin/env bash
# constellation-MaxQ — from stock to MaxQ in one command. HOME-only persistence.
set -euo pipefail
PREFIX="${MAXQ_HOME:-$HOME}"; BIN="$PREFIX/bin"; CONFIG="$PREFIX/.config/maxq"; ICON_SRC="$CONFIG/icons-src"
RAW_BASE="${MAXQ_RAW_BASE:-https://raw.githubusercontent.com/0sm0s1z/constellation-MaxQ/main}"
mkdir -p "$BIN" "$CONFIG/api-src/ui" "$CONFIG/theme/src" "$ICON_SRC"
